 function [u, Kcond] = SolverWaves(problem, domain, mesh, disc, varargin)
    ...
    % Boundary unpacking % THIS NEEDS TO BE REDONE!
    top     = mesh.top;
    left    = mesh.left;
    right   = mesh.right;
    bot     = mesh.bot;

    % Boundary elms unpacking % REDO THIS AS WELL
    ...
    % gradu * gradv
    gradu_gradv = kron(Yb.d0d0, Xb.d1d1) + kron(Yb.d1d1, Xb.d0d0);
    u_v = kron(Yb.d0d0, Xb.d0d0);
    % gradu * gradv * c^2 * (BETA - XI)
    opQ = kron(Tb.d0d0, gradu_gradv) * c^2 * (BETA + (2 - d) * XI);
    % ut * vt * (BETA + XI)
    opQ = opQ + kron(Tb.d1d1, u_v) * (BETA + d * XI);
    % Lap u * Lap v
